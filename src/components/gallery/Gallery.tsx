...
const Gallery: React.FC<GalleryProps> = ({ styleNum }) => {
  ...
  return (
    <div
      ref={galleryRef}
      className={`gallery-section gallery-1 ${parentClass} `}
    >
      <GallerySwiper
        data={galleryData.slice(0, 8)}
        galleryData={galleryData}
        galleryClass={'swiper_gallery'}
      />
      <GallerySwiper
        data={galleryData.slice(6, -1)}
        galleryData={galleryData}
        galleryClass={'swiper_gallery2'}
      />
    </div>
  );
};
