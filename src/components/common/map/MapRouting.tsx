...
export default function MapRouting(): React.ReactElement {
  const map = useMap();
  ...
  useEffect(() => {
    ...
    if (locationArray.length) {
      const routingControl = L.Routing.control({
        waypoints: [
          L.latLng(locationArray[0], locationArray[1]),
          L.latLng(locationLatLng[0], locationLatLng[1]),
        ],
      ...
      if (routingControl) {
        const leaflet_container = document.querySelector(
      ...
      const closeBtn = document.querySelector('.leaflet-routing-collapse-btn') as HTMLElement;
      if (closeBtn) {
        let check = 0;
        closeBtn.addEventListener('click', () => {
          ...
          if (leaflet_container) {
            if (check === 0) {
              ...
              leaflet_container.classList.remove(
