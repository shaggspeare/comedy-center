...
interface MenuItem {
  ...
  name: string;
  path: string;
  isDropdown: MenuItemDropdown[] | false;
}

const NavbarSeven: React.FC = () => {
              ...
              <div className="d-none d-lg-block">
                <div className="d-flex gap-30 align-items-center">
                  <ul className="menu-list navbar-nav mb-2 me-2 mb-lg-0">
                    {/*{*/}
                    {/*    (menuList as MenuItem[]).map(({ id, isDropdown, name, path, }) => {*/}
                    {/*        return (*/}
                    {/*            <li key={id}*/}
                    ...
                    {/*                                    return (*/}
                    {/*                                        <li key={id}><Link className="dropdown-item" href={path} target="_self" aria-label="single-pages">{name}</Link></li>*/}
                    {/*                                    )*/}
                    {/*                                })*/}
                    {/*                            }*/}
                    {/*                        </ul>*/}
              ...
              <div className="d-none d-lg-block">
                <div className="d-flex gap-40 align-items-center">
                  <Link
                    href="#"
                    data-bs-toggle="offcanvas"
                    data-bs-target="#offcanvasShop"
                    aria-controls="offcanvasShop"
                    aria-expanded="false"
                    aria-label="Toggle navigation"
                  >
                    <span className="shop-icon-xl">
                      {' '}
                      <ShopIcon height={'46'} width={'42'} />{' '}
                    </span>
                  </Link>
                  <ButtonFill
                    className={' btn-rounded gap-2'}
                    link={'#pricing'}
                  >
