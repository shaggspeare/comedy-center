...
const PricingCard: React.FC<PricingCardProps> = ({
  ...
  let isBtnOutline: boolean = false;
  let featureClass: string = '';
  let cardNameClass: string = '';
  ...
  return (
    <div
      className={`pricing-wrapper bg-ticket ${prantClass} position-relative`}
    >
      <div className="pricing-top-content">
        <div className="pricing-icon mb-10">{icon}</div>
        <h3 className={`fw-semibold pricing-title ${cardNameClass}`}>
          {card_name}
        </h3>
        <h3 className="mb-40">
          <span className={`display-2 fw-extra-bold me-10 ${priceClass}`}>
            {price}
          </span>
          <span className="fw-semibold pricing-subtitle">/ Person</span>
        </h3>
