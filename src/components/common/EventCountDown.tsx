...
const EventCountDown: React.FC<EventCountDownProps> = ({ styleNum }) => {
  ...
  let countdownGradient: string;
  let isBg: string;
  let labelClass: string;

  switch (styleNum) {
    case 0:
      prantClass = 'pt-4 pt-lg-30 pt-xxl-40';
        ...
        <div className="container">
          ...
          <div className={`countdown ${countdownGradient}`}>
            <div className="row row-cols-2 row-cols-lg-3 row-cols-xl-4 justify-content-between align-items-center">
              ...
              <Col
                name={'Minutes'}
                time={currentTime.currentMinutes}
                labelClass={labelClass}
                timeClass={timeClass}
                id="minutes"
              />
              <Col
                name={'Seconds'}
                time={currentTime.currentSeconds}
                labelClass={labelClass}
                timeClass={timeClass}
                id="seconds"
              />
...
const Col: React.FC<ColProps> = ({ time, name, labelClass, timeClass, id }) => {
  return (
    <div className="col">
      <div className="countdown-item">
        <span className={`countdown-number  ${timeClass}`} id={id}>
          {time}
        </span>
        <span className={`countdown-label text-opacity ${labelClass}`}>
          {' '}
          <WordOpacity paragraph={name} />{' '}
        </span>
      </div>
    </div>
  );
